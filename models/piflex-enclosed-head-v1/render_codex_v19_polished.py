"""Render exterior and routing views of the polished Codex V19 assembly."""
...
structure = next(obj for obj in meshes if obj.name.startswith("PiFlex Codex V19"))
shell.data.materials.clear()
shell.data.materials.append(material("Exact screen shell", (0.06, 0.23, 0.38)))
structure.data.materials.clear()
structure.data.materials.append(material("Polished rear and mount", (0.88, 0.25, 0.055)))
...
camera.data.ortho_scale = 355
scene.camera = camera

views = {
    "rear": (centre + Vector((0.0, -430.0, 0.0)), centre, 355),
    "front": (centre + Vector((0.0, 430.0, 0.0)), centre, 355),
...
for suffix, (location, target, scale) in views.items():
    ...
    scene.render.filepath = str(HERE / f"piflex-codex-v19-polished-{suffix}.png")
    bpy.ops.render.render(write_still=True)

print("Rendered PiFlex Codex V19 polished assembly")
